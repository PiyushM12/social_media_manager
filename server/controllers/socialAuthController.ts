...
const getOrCreateZernioProfile = async (user:any) : Promise<string> => {
    try {
       ...
       return pid;
        ...
        console.error("getOrCretaeZernioProfile Error:",error?.message||error);
        throw error;
...
export const generateAuthUrl = async (req: AuthRequest, res: Response) : Promise<void>=> {
    try {
        ...
        const data = result.data as any;
        console.log("gtConectUrl response:", JSON.stringify(data,null,2))
...
export const syncAccounts = async (req: AuthRequest, res:Response): Promise<void>=>{
    try{
        ...
        const data = result.data as any;
        const zernioAccounts : any[]= data?.acconts||(Array.isArray(data)?data:[]);
